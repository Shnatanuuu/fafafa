...
UI_TEXTS = {
    ...
    "zh": {
        ...
        "description":       "描述（销售、技术及品控经理填写）",
        "cap_desc":          "纠正措施计划（CAP）描述",
        "sales_comments":    "销售意见",
        "tech_comments":     "技术部意见",
        "qc_comments":       "品控经理意见",
        "conclusion_text":   "总体结论",
        "qc_signature":      "品控经理签名",
        "qc_date":           "品控日期",
        "select_location":   "选择评估地点",
        "ui_lang":           "界面语言",
        "pdf_lang":          "PDF报告语言",
        "local_time":        "本地时间",
...
def draw_page_frame(c, page_num, total_pages, pdf_lang, city, city_zh, gen_time):
    ...
    header_r = "生产风险评估报告" if pdf_lang == "zh" else "PRODUCTION RISK ASSESSMENT REPORT"
...
def generate_pdf():
    ...
    # ── Two-pass for correct page total ──────────────────────────────────────
    def _build(buf_out, total_pages_known):
        ...
        c.drawString(MARGIN_L + 24, y - 60,
                     "生产风险评估报告" if pdf_lang == "zh" else "PRODUCTION RISK ASSESSMENT REPORT")
        ...
        pairs = [
            (loc("PO / Order Number","PO / 订单编号"), po_number or '—',
             loc("Brand / Trademark","品牌 / 商标"),   tx(brand)  or '—'),
            (loc("Style / Model","款式 / 型号"),        tx(style_val) or '—',
             loc("Sales / Business","销售 / 业务"),     tx(sales)  or '—'),
            (loc("Factory Name","工厂名称"),            tx(factory) or '—',
             loc("Assessment Date","评估日期"),         adate_str),
        ]
        ...
        y = draw_section_header(c, y, loc("2. RISK ASSESSMENT MATRIX","2. 风险评估矩阵"), pdf_lang)
        ...
        y = draw_text_block(c, y, loc("Sales Comments:","销售意见："),
                            tx(sales_comments), pdf_lang, C_ACCENT2)
        y = maybe_new_page(y, 80)
        y = draw_text_block(c, y, loc("Technical Comments:","技术部意见："),
                            tx(tech_comments), pdf_lang, C_ACCENT2)
        y = maybe_new_page(y, 80)
        y = draw_text_block(c, y, loc("QC Manager Comments:","品控经理意见："),
                            tx(qc_comments), pdf_lang, C_ACCENT2)
        y -= 10

        # Conclusion
        y = maybe_new_page(y, 120)
        y = draw_section_header(c, y, loc("4. CONCLUSION & APPROVALS","4. 结论与批准"), pdf_lang)
        y = draw_text_block(c, y, loc("Overall Conclusion:","总体结论："),
        ...
        conf = loc(
            ...
            "本报告为Grand Step (H.K.) Ltd机密文件，未经授权禁止分发。"
        )
        c.setFillColor(C_GREY_TEXT); c.setFont(fn_r, 7.5)
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.text_input(f"📄 {t('po_number')}", placeholder="PO-2024-001",                 key="po_number")
        st.text_input(f"👕 {t('style')}",     placeholder="Model XYZ-2024",              key="style_val")
        st.text_input(f"🏭 {t('factory')}",   placeholder="ABC Manufacturing Co., Ltd.", key="factory")
    with c2:
        st.text_input(f"🏷️ {t('brand')}",     placeholder="Brand Name",                  key="brand")
        st.text_input(f"👔 {t('sales')}",     placeholder="Sales Representative Name",   key="sales")
        st.date_input(f"🕐 {t('assessment_date')}", datetime.now().date(),               key="assessment_date")

# ════════════════════════════════════════════════════════════════════════════
with tab2:
    st.markdown(f'<div class="section-header">🔍 {t("risk_assessment")}</div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
with tab3:
    st.markdown(f'<div class="section-header">✍️ {t("dept_comments")}</div>', unsafe_allow_html=True)

    st.markdown(f"#### 👔 {t('sales_comments')}")
    ...
    st.markdown(f'<div class="section-header">✅ {t("conclusion")}</div>', unsafe_allow_html=True)
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.text_input(f"👁️ {t('qc_signature')}", placeholder="QC Manager name", key="qc_signature")
    with c2:
        st.date_input(f"📅 {t('qc_date')}", datetime.now().date(), key="qc_date")
